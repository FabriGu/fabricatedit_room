export class WorkCard {
  ...
  createRowCard() {
    ...
    // carouselTrack.className = "carousel-track carousel-items"; // Add carousel-items class
    ...
    card.addEventListener("mouseleave", function () {
      ...
      const opaque = carouselWrapper.querySelector(
...
export class WorkLoader {
  constructor(containerId) {
    ...
    this.containerType = this.container.id.substring(
      this.container.id.length - 5
    );
    ...
    try {
      ...
      for (const work of worksIndex.works) {
        const dataResponse = await fetch(
          `../${this.containerType}/data/${work.dataFile}`
        );
    ...
    for (const [title, { items, workData }] of this.pendingCarouselItems) {
      ...
      for (const file of items) {
        ...
        const carousel = document.querySelector(
          `[data-title="${title}"] .carousel-track`
        );
        if (carousel) {
  ...
  renderWorks() {
    if (this.carousels && this.carousels.length > 0) {
      this.carousels.forEach((scroller) => {
    ...
    this.works.forEach((work) => {
      ...
      // Initialize carousel scrolling if this is a row card
        if (work.data.type === "row") {
            console.log('Setting up carousel for row type:', work.data.title);
            
            // Give the browser a moment to render the card before initializing the scroller
            setTimeout(() => {
            const carouselWrapper = card.querySelector('.carousel-wrapper');
            if (carouselWrapper) {
                console.log('Found carousel wrapper for:', work.data.title);
                const scroller = new CarouselScroller(carouselWrapper, {
                scrollSpeed: 1, // Increased speed for visibility
                pauseOnHover: true,
                scrollPadding: 20
                });
                
                // Store reference to the scroller for cleanup
                this.carousels.push(scroller);
            } else {
                console.warn('No carousel wrapper found for:', work.data.title);
            }
...
class CarouselScroller {
  constructor(carouselElement, options = {}) {
    ...
    this.scrollContainer =
      this.carousel.querySelector(".carousel-items") || this.carousel; // Adjust selector based on your DOM structure

    // Configuration options with defaults
    this.options = {
      scrollSpeed: options.scrollSpeed || 1, // pixels per frame
      pauseOnHover:
        options.pauseOnHover !== undefined ? options.pauseOnHover : true,
      scrollPadding: options.scrollPadding || 20, // padding to detect scroll boundaries
      ...options,
    };
    ...
    this.scrollDirection = 1; // 1 for right-to-left, -1 for left-to-right
  ...
  init() {
    ...
    if (this.options.pauseOnHover) {
      this.carousel.addEventListener("mouseenter", this.onMouseEnter);
      this.carousel.addEventListener("mouseleave", this.onMouseLeave);
      this.carousel.addEventListener("touchstart", this.onMouseEnter);
      this.carousel.addEventListener("touchend", this.onMouseLeave);
    }

    // Listen for manual scroll events
    this.scrollContainer.addEventListener("scroll", this.onUserScroll);
  ...
  animate() {
    if (!this.isScrolling || this.isPaused || this.isUserScrolling) {
      this.animationFrameId = requestAnimationFrame(this.animate);
      return;
    }
    ...
    // Check if we've reached the beginning or end of the carousel
    const isAtStart = scrollLeft <= this.options.scrollPadding;
    const isAtEnd =
      scrollLeft >= scrollWidth - containerWidth - this.options.scrollPadding;

    // Reverse direction if we've reached the boundaries
    if (
      (isAtStart && this.scrollDirection === -1) ||
      (isAtEnd && this.scrollDirection === 1)
    ) {
    ...
    const newScrollLeft =
      scrollLeft + this.options.scrollSpeed * this.scrollDirection;
  ...
  onMouseLeave() {
    this.isPaused = false;
    // Reset user scrolling flag when mouse leaves
    setTimeout(() => {
      this.isUserScrolling = false;
    }, 100);
  }
  ...
  onUserScroll() {
    // Detect if this is a user-initiated scroll by checking if position changed drastically
    const currentScrollPosition = this.scrollContainer.scrollLeft;
    const scrollDifference = Math.abs(
      currentScrollPosition - this.lastScrollPosition
    );

    if (scrollDifference > this.options.scrollSpeed * 2) {
      this.isUserScrolling = true;
      clearTimeout(this.scrollEndTimer);

      // Set a timer to detect when the user stops scrolling
      this.scrollEndTimer = setTimeout(this.onScrollEnd, 150);
    }
  ...
  destroy() {
    ...
    this.carousel.removeEventListener("touchstart", this.onMouseEnter);
    this.carousel.removeEventListener("touchend", this.onMouseLeave);
    this.scrollContainer.removeEventListener("scroll", this.onUserScroll);
  }
}
