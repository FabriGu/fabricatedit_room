...
function createMarioStar(size = 1, points = 5, innerRadius = 0.4, bulge = 1.55) {
    ...
    // Create the front face vertices (slightly bulged)
    const frontZ = bulge * size;
    const backZ = -bulge * size;
    
    // Center point (front bulge)
    const centerFrontIndex = 0;
    vertices.push(0, 0, frontZ);
    
    // Center point (back)
    const centerBackIndex = 1;
    vertices.push(0, 0, backZ);
    
    // Create star points
    for (let i = 0; i < points; i++) {
        const angle = (Math.PI * 2 * i / points) - Math.PI / 2; // Start from top
        ...
        const innerX = Math.cos(innerAngle) * (size * innerRadius);
        const innerY = Math.sin(innerAngle) * (size * innerRadius);
        vertices.push(innerX, innerY, 0);
    }
    
    // Create triangles for the front face (bulged side)
    for (let i = 0; i < points; i++) {
        const outerPointIndex = 2 + i * 2;
        const innerPointIndex = 3 + i * 2;
        const nextOuterPointIndex = 2 + ((i + 1) % points) * 2;
        
        // Triangle from center to outer point to inner point
        indices.push(centerFrontIndex, outerPointIndex, innerPointIndex);
        
        // Triangle from center to inner point to next outer point
        indices.push(centerFrontIndex, innerPointIndex, nextOuterPointIndex);
    }
    ...
    for (let i = 0; i < points; i++) {
        ...
        // Triangle from center to inner point to outer point (reversed winding)
        indices.push(centerBackIndex, innerPointIndex, outerPointIndex);
        
        // Triangle from center to next outer point to inner point (reversed winding)
        indices.push(centerBackIndex, nextOuterPointIndex, innerPointIndex);
    }
    
    // Create side triangles to connect front and back
    for (let i = 0; i < points; i++) {
        const outerPointIndex = 2 + i * 2;
        const innerPointIndex = 3 + i * 2;
        const nextOuterPointIndex = 2 + ((i + 1) % points) * 2;
        
        // Two triangles to form a quad for each side
        indices.push(outerPointIndex, centerFrontIndex, centerBackIndex);
        indices.push(outerPointIndex, centerBackIndex, outerPointIndex);
        
        indices.push(innerPointIndex, centerFrontIndex, centerBackIndex);
        indices.push(innerPointIndex, centerBackIndex, innerPointIndex);
        
        // Connect outer point to inner point on both front and back
        indices.push(outerPointIndex, innerPointIndex, centerFrontIndex);
        indices.push(outerPointIndex, innerPointIndex, centerBackIndex);
    }
...
// Alternative implementation using LatheGeometry for a smoother star with bulge
function createSmoothMarioStar(size = 1, points = 5, innerRadius = 0.4, bulge = 0.55) {
    ...
    for (let i = 0; i < points * 2; i++) {
        const isOuter = i % 2 === 0;
        const radius = isOuter ? size : (innerRadius * size);
    ...
    // Extrusion settings with bulge
    const extrudeSettings = {
        steps: 1,
        depth: 0.1 * size,
        bevelEnabled: true,
        bevelThickness: bulge * size,
        bevelSize: 0.05 * size,
        bevelOffset: 0,
        bevelSegments: 5
    };
...
function createMarioStarMaterial(color = 0xFFDF00) {
...
export class DitheredRoom {
  ...
  setup() {
    ...
    this.controls = new EnhancedControls(
      this.camera,
...
createDitherPattern() {
  ...
  setupInteractionSystem() {
    ...
    this.container.addEventListener("touchstart", handleInteraction, {
      passive: false,
    });
  ...
  async goToViewpoint(name) {
    ...
    const heightOffset = Math.min(
      startPos.distanceTo(viewpoint.position) * 0.25,
      2
    );
  ...
  addInteractiveItem(
    ...
    const getGlowColor = (type) => {
      switch (type) {
        case "viewpoint":
          return new THREE.Color(0x8a00c2); // Purple
        case "info":
          return new THREE.Color(0xebc934); // Red
        default:
  ...
  onWindowResize() {
    ...
    //     window.innerHeight
    //   );
    //   this.ditherPass.uniforms.ditherScale.value =
...
class BillboardText extends THREE.Object3D {
  ...
  update(camera) {
    ...
    const directionToCamera = new THREE.Vector3()
      .subVectors(camera.position, this.sphereCenter)
      .normalize();
